...
PostgreSQL 默认使用 TLS 完整验证。同机 Docker 或隔离私网中未开启数据库 SSL 时，可选择“可信私网明文”，并明确确认密码和数据不加密传输。该模式只接受 IPv4 私网或 IPv6 ULA 地址，拒绝公网、回环、链路本地和云元数据地址；连接测试后固定解析出的数字地址，保存配置和重启时再次核对原主机解析，解析变化时拒绝继续连接。容器重建时应保留数据库网络地址；若地址变化导致启动被拒绝，先恢复原地址，不能删除安装标记绕过校验。网站安装页面仍必须通过 HTTPS 访问，数据库明文选项不会放宽网页凭据传输要求。1Panel 部署可让后端加入 PostgreSQL 所在 Docker 网络，通过数据库容器名称连接，无需暴露数据库公网端口。
...
PostgreSQL TLS 默认 `VERIFY_FULL`，也可选择 `VERIFY_CA`；`REQUIRE` 不校验证书和主机名，必须显式确认风险。`DISABLE` 仅在目标全部解析为可信私网地址且明确确认明文风险时允许，仍禁止自动降级的 `PREFER`。Redis 默认使用 JVM 系统信任证书，也可在页面提供私有 CA；只有解析到可信私网地址时才允许显式确认后关闭 Redis TLS。
...
PostgreSQL 和 Redis 都接管并重启验证后，向导才执行数据库结构、站点单例、上传目录和 Redis 实际读写检查，然后创建站点信息与唯一首位网站管理员。数据库连接测试的 ticket 不能替代后续结构检查：保存数据库配置时重新检查目标，环境检查与最终管理员初始化事务也会复核当前 PostgreSQL 定义；定义不符时拒绝完成安装，不写入管理员或安装完成标记，恢复定义后可重新检查和提交。匿名状态接口不会探测外部资源或公开组件细节。任一依赖不可达时环境检查失败，不允许完成安装；安装完成后的健康检查也会失败关闭，但不会重新开放换库或换 Redis 入口。安装完成后写入永久完成标记，后续所有修改型安装接口拒绝再次初始化；容器重启不会重新开放入口。
...
- 分类、书签、自定义链接和搜索引擎的普通编辑支持按字段更新，省略或 `null` 表示保持，可选文本用空字符串清空；显隐和排序继续只写目标字段。内置后台仅提交实际改变的字段，避免旧表单覆盖其他请求已保存的内容。外部集成也应省略未修改字段；两个请求显式修改同一字段时仍按成功提交顺序生效，不提供版本冲突检测。记录已删除时编辑返回 `404`，写入失败会连同公开缓存版本一起回滚。
- API 的 ID、排序、版本、安装端口和 Redis 逻辑库/超时等整数输入拒绝小数及浮点形式（例如 `1.5`、`1.0`、`1e0`），返回 `400`，不会截断后修改另一条记录或连接错误的目标。整数数字字符串仍兼容；ZIP 整数字段遵循相同的浮点拒绝规则。
...
三类批量请求均先完整校验后写入，任一 ID 非法、重复或不存在时整批不生效。分类和书签排序支持超过 1000 项：一次提交完整顺序，后端在同一事务内分段查询加锁后统一保存，任一写入失败都会整批回滚；请求仍受部署层请求体大小与超时限制。批量移动仍为单次最多 1000 项的幂等操作，可安全重试，响应只包含本次请求的书签，内置前端会在成功后重新获取完整列表。
...
`ops/rollback-release.sh <后端镜像摘要引用> <前端镜像摘要引用>` 仅切换已经拉取到本机的应用镜像；两个参数必须分别使用 `ghcr.io/<owner>/yunlume-backend@sha256:<摘要>` 与 `ghcr.io/<owner>/yunlume-frontend@sha256:<摘要>`，不会接受 `latest`、版本标签或其他可变引用。它不修改数据库、`yunlume_database_config` 或上传卷。执行前必须同时提供 `CONFIRM_ROLLBACK=ROLLBACK-RELEASE` 和 `CONFIRM_EXTERNAL_DATABASE_BACKUP=EXTERNAL-DATABASE-BACKUP-VERIFIED`。

回滚验收要求每次 `compose ps` 和 `docker inspect` 查询成功，前后端各返回单个合法容器 ID，健康状态精确为 `healthy`；命令失败时，即使输出中带有 ID 或 `healthy` 也不能作为成功依据。随后必须通过 `/healthz`、后端 `UP` 和安装状态 `COMPLETED` 检查，并在 HTTP 检查后再次确认容器健康，才报告代码回滚完成。

目标版本失败后会尝试恢复执行前的镜像；即使原版本恢复并通过相同验收，本次操作仍以非零状态退出。原版本恢复失败或结果无法确认时退出 `2`，保留现有 `.env.release-rollback-*` 备份供人工核对；恢复材料缺失也属于恢复失败，不能报告成功。代码镜像回滚不会撤销已提交的数据库迁移或数据导入，不能代替数据库或连接配置恢复，也不能保证新 schema 与旧应用兼容。
