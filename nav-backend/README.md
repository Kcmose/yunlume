...
生产环境只支持外部 PostgreSQL 14 或更高版本。安装向导要求专用、非超级用户角色，强制 TLS，并通过一次性连接票据、实例 UUID 与后端私有运行配置防止改库和身份漂移。接管现有未安装结构前，除对象和非空约束外还会只读核对列类型、长度、默认值、identity/generated 属性及排序规则，同名但定义不兼容的列不能通过连接票据检查。项目不提供或启动 PostgreSQL 服务，也不会代替数据库服务商执行备份与恢复。
