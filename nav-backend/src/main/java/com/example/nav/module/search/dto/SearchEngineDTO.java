...
public record SearchEngineDTO(
        @NotBlank(groups = CreateValidation.class, message = "搜索引擎名称不能为空")
        @Pattern(regexp = "(?s).*[^\\p{javaWhitespace}].*", message = "搜索引擎名称不能为空")
        ...
        @NotBlank(groups = CreateValidation.class, message = "搜索地址模板不能为空")
