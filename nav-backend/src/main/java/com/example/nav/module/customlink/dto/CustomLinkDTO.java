...
public record CustomLinkDTO(
        @NotBlank(groups = CreateValidation.class, message = "链接文字不能为空")
        @Pattern(regexp = "(?s).*[^\\p{javaWhitespace}].*", message = "链接文字不能为空")
        @Size(max = 50, message = "链接文字不能超过 50 个字符")
        String title,

        @NotBlank(groups = CreateValidation.class, message = "链接地址不能为空")
        @Pattern(regexp = "(?s).*[^\\p{javaWhitespace}].*", message = "链接地址不能为空")
        @Size(max = 500, message = "链接地址不能超过 500 个字符")
        String url,

        @NotBlank(groups = CreateValidation.class, message = "显示位置不能为空")
