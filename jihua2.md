...
5. 验证完成后不能重新安装。
6. 验证服务重启和重复运行安装脚本。
7. 验证两种模式升级、失败回滚和跨兼容代际拒绝；同代际回退需等待第二个支持兼容代际的正式版本后再做真实 Release 闭环。

上述发布、Docker/宿主机安装升级、`latest` 语义、兼容代际和跨代际保护均已完成；同代际旧版本回退受 `v1.0.6` 旧安装器限制，不作为 `v1.0.7` 交付阻塞项。详细证据与边界见 [`ACCEPTANCE.md`](ACCEPTANCE.md)。构建服务器上的临时 Docker 验收部署已删除，Maven、npm 等构建依赖缓存按约定保留。
