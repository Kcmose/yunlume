...
当前轮不执行 `install.sh`，不启动 yunlume 应用，也不配置 PostgreSQL 或 Redis。代码推送 GitHub 后，先由 GitHub Actions 验证主分支并发布前后端 `latest`/提交 SHA 镜像；后续另行确认创建正式版本标签后，再由 GitHub Actions 生成 Release 资产。正式 Release 可公开下载后，单独执行以下一键安装验收：
